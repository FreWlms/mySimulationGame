...
public class Movie extends JComponent {
    ...
    public int getTargetFramesPerSecond(){return this.timer.getTargetFrameRate();}

    public void setTargetFrameRate(int targetFrameRate){this.timer.setTargetFrameRate(targetFrameRate);}
    ...
    private void renderCreatures() {
    	...
    	for (CreatureA creatureA : sim.getWorld().getPopulationA()) 
    		this.bfiRenderer.renderCreature(bufferedImage, creatureA.getPosition(), Color.RED);
    	
    	for (CreatureB creatureB : sim.getWorld().getPopulationB())
    		this.bfiRenderer.renderCreature(bufferedImage, creatureB.getPosition(), Color.BLUE);
    } 
}
